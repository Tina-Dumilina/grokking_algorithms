...
Динамическое программирование - это метод решения сложных задач, разбиваемых на подзадачи, которые решаются в первую очередь. Динамическое программирование применяется для оптимизации какой-либо характеристики при заданных органичениях. Например, в задаче о рюкзаке требуется максимизировать стоимость отобранных предметов с органичениями по емкости рюкзака. 
...
2. Если они совпадают, то значение равно значению ячейки наверху слева + 1.
...
1. Если буквы не совпадают, выбрать более значение.
2. Если они совпадают, то значение равно значению ячейки наверху слева + 1.
...
if word_a[i] === word_b[j]
  ...
  cell[i][j] = max(cell[i][j], cell[i][j-1])
```
